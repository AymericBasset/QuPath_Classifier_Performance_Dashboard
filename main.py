...
if uploaded_file:
    ...
    if TRAIN_TEST_COLUMN not in df.columns or PROBA_COLUMN not in df.columns or CATEGORY_COLUMN not in df.columns:
        ...
        TRAIN_CLASS_NAME = st.selectbox("Select Train Class:", unique_values)
        ...
        def get_summary(df, positive_classes):
            ...
            summary_data = {
                ...
                'all': [
                    ...
                    len(df) - sum(df['Classification'].isin(positive_classes)),
                    len(df)
                ]
            }
        ...
        auc_mean_train, auc_std_train = np.mean(aucs_train), np.std(aucs_train)
        ...
        with a:
            ...
            contingency_table = [[pos_train, neg_train], [pos_test, neg_test]]
